...
import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Database, Edit3, Trash2, Save, X, Loader2, CheckCircle2, ArrowLeft, BookOpen, CheckSquare, AlignLeft, LayoutList, AlertTriangle, Send } from 'lucide-react';
import api from '../api/axios';
...
const QuestionBankTab = ({ courseId }) => {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const [examsRes, quizzesRes, assignmentsRes] = await Promise.all([
        api.get(`/api/v1/exams/questions/course/${courseId}`).catch(() => ({ data: [] })),
        api.get(`/api/quiz/questions/course/${courseId}`).catch(() => ({ data: [] })),
        api.get(`/api/assignments/questions/course/${courseId}`).catch(() => ({ data: [] }))
      ]);

      // 🔥 THE FIX: DATA NORMALIZER + GHOST SHIELD 🛡️
      const processQuestions = (data, type) => {
        let arr = Array.isArray(data) ? data : (data?.questions ? data.questions : (data ? [data] : []));
        
        return arr
          .filter(q => q && (q.examQuestionId || q.questionId))
          // 🛡️ THE SHIELD: If the text is blank or missing, drop it instantly!
          .filter(q => {
              const text = q.questionText || q.question || "";
              return typeof text === 'string' && text.trim() !== ""; 
          })
          .map(q => {
            const actualId = q.examQuestionId || q.questionId;
            const pId = q.midtermId || q.quizId || q.assignmentId || 'unassigned'; 
            return {
                ...q,
                examQuestionId: actualId, 
                question: q.questionText || q.question,
                referenceAnswer: q.referenceAnswer || q.idealAnswer || q.answer || "Pending",
                uniqueReactKey: `${type}-${pId}-${actualId}`,
                parentType: type,
                parentId: pId === 'unassigned' ? null : pId,
      ...
      const allQuestions = [
        ...processQuestions(examsRes.data, 'midterm'),
        ...processQuestions(quizzesRes.data, 'quiz'),
        ...processQuestions(assignmentsRes.data, 'assignment')
      ];
  ...
  const handleDelete = async (questionId, parentType) => {
    if (!window.confirm("Are you sure you want to delete this AI-generated question?")) return;
    try {
      let deleteEndpoint = `/api/v1/exams/questions/${questionId}`;
      if (parentType === 'quiz') deleteEndpoint = `/api/quiz/questions/${questionId}`;
      if (parentType === 'assignment') deleteEndpoint = `/api/assignments/questions/${questionId}`;

      await api.delete(deleteEndpoint);
      setQuestions(questions.filter(q => q.uniqueReactKey !== `${parentType}-${questionId}`));
    } catch (err) {
      alert("Failed to delete question. Endpoint may be missing.");
    }
  };

  const handlePublishParent = async (q) => {
    if (!q.parentId || !q.parentType) {
        alert("Cannot publish: Missing parent Assessment ID from backend.");
        return;
    }
    setPublishingId(q.uniqueReactKey);
    try {
        const role = localStorage.getItem('role') || 2;
        let publishEndpoint = "";
        
        if (q.parentType === 'assignment') publishEndpoint = `/api/assignments/${q.parentId}/publish`;
        else if (q.parentType === 'midterm') publishEndpoint = `/api/v1/exams/${q.parentId}/publish`;
        else publishEndpoint = `/api/quiz/${q.parentId}/publish`;

        await api.put(publishEndpoint, {}, { headers: { 'X-User-Role': role } });
        
        setQuestions(prev => prev.map(item => 
            (item.parentId === q.parentId && item.parentType === q.parentType) 
            ? { ...item, isPublished: true } 
            : item
        ));
        
        alert("✅ Success! The assessment is now LIVE and visible to students.");
    } catch (e) {
        alert("❌ Failed to publish. Ensure @PutMapping is in your Java Controller.");
    } finally {
  ...
  const startEditing = (q) => {
    ...
    if (isM) {
      try {
        ...
        if (Array.isArray(rawOpts) && rawOpts.length > 0) {
          parsedOptions = [
            rawOpts[0] || '', rawOpts[1] || '', rawOpts[2] || '', rawOpts[3] || ''
          ];
        }
      } catch (e) {
        console.error("Parse error", e);
  ...
  const saveEdit = async () => {
    try {
      ...
      payload.questionText = payload.question;
      ...
      delete payload.parsedOptions;
      delete payload.isMCQ;
      delete payload.uniqueReactKey;

      let updateEndpoint = `/api/v1/exams/questions/${editForm.examQuestionId}`;
      if (editForm.parentType === 'quiz') updateEndpoint = `/api/quiz/questions/${editForm.examQuestionId}`;
      if (editForm.parentType === 'assignment') updateEndpoint = `/api/assignments/questions/${editForm.examQuestionId}`;

      await api.put(updateEndpoint, payload);
      
      setQuestions(questions.map(q => q.uniqueReactKey === editForm.uniqueReactKey ? { ...payload, optionsJson: payload.optionsJson } : q));
      setIsEditingMode(false);
      setEditForm({});
    } catch (err) {
      alert("Failed to update question.");
          ...
          <AnimatePresence mode="wait">
            ...
            /* ========================================== */
              ...
              >
                ...
                <div className="space-y-4">
                {displayQuestions.length > 0 ? displayQuestions.map((q, index) => {
                      const isM = checkIsMCQ(q);
                    
                    return (
                      // 🔥 USING UNIQUE KEY TO PREVENT CRASHES
                      <div 
                        key={q.uniqueReactKey || `q-card-${index}`}
                        className={`bg-white dark:bg-slate-900 p-6 rounded-[2rem] border transition-colors shadow-sm hover:shadow-md ${!q.isPublished ? 'border-yellow-300 dark:border-yellow-700/50' : 'border-slate-100 dark:border-slate-800 hover:border-emerald-200 dark:hover:border-emerald-800'}`}
                      >
                        <div className="flex justify-between items-start gap-6">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center flex-wrap gap-3 mb-3">
                              ...
                              {q.isPublished ? (
                                <span className="px-2.5 py-1 rounded-md bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 text-[9px] font-black uppercase tracking-widest flex items-center gap-1">
                                    <CheckCircle2 className="w-3 h-3"/> Published
                                </span>
                              ) : (
                                <span className="px-2.5 py-1 rounded-md bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 text-[9px] font-black uppercase tracking-widest flex items-center gap-1">
                                    <AlertTriangle className="w-3 h-3"/> Draft
                                </span>
                              )}

                              <span className="px-2 py-1 rounded-md text-[8px] font-black text-slate-400 uppercase tracking-widest border border-slate-200 dark:border-slate-700">
                                Source: {q.parentType} (ID: {q.parentId})
                              </span>
                            </div>
                            
                            {/* 🔥 NOW USING THE NORMALIZED QUESTION TEXT */}
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-200 leading-relaxed mb-4">
                              {q.question}
                            </p>
                            ...
                            <div className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-100 dark:border-slate-700/50">
                              ...
                              {isM 
                                ? <span className="text-emerald-600 dark:text-emerald-400">Answer Key: {q.correctAnswer}</span>
                                : <span className="text-fuchsia-600 dark:text-fuchsia-400 line-clamp-1">Ref: {q.referenceAnswer}</span>
                              }
                            </div>
                          ...
                          <div className="flex flex-col gap-2 shrink-0">
                            {!q.isPublished && (
                                <button 
                                    onClick={() => handlePublishParent(q)}
                                    disabled={publishingId === q.uniqueReactKey}
                                    className="p-4 text-emerald-500 hover:text-white hover:bg-emerald-500 dark:hover:bg-emerald-600 rounded-2xl transition-all shadow-sm border border-emerald-100 dark:border-emerald-800/50"
                                    title="Approve & Publish this Assessment"
                                >
                                    {publishingId === q.uniqueReactKey ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
                                </button>
                            )}
                            ...
                            <button 
                              // 🔥 Passing parentType so deletion routes to the right DB!
                              onClick={() => handleDelete(q.examQuestionId, q.parentType)} 
                              className="p-4 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-2xl transition-all"
                              title="Delete this Question"
                            >
                              <Trash2 className="w-5 h-5" />
                    ...
                    <div className="text-center p-12 bg-slate-50 dark:bg-slate-800/50 rounded-[3rem] border border-dashed border-slate-200 dark:border-slate-700">
                      <Database className="w-12 h-12 text-slate-300 mx-auto mb-4" />
                      <p className="font-black text-slate-400 uppercase tracking-widest text-sm">No {filter !== 'ALL' ? filter : ''} Questions Found</p>
                      <p className="text-xs text-slate-400 mt-2">Generate assessments in the Builder Tab first.</p>
                    </div>
                  )}
                </div>
              </motion.div>
