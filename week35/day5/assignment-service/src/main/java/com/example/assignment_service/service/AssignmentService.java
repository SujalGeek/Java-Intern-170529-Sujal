...
//    }
    ...
    public Double getAverageForStudent(Long studentId, Long courseId) {
        // We query the assignment_answer table linked through assignment_attempt
    	// Corrected SQL for Quiz Service
    	// Corrected SQL for Assignment Service
//    	String sql = "SELECT AVG(percentage) FROM assignment_attempt WHERE student_id = ? AND assignment_id IN (SELECT assignment_id FROM assignment WHERE course_id = ?)";
    	// Corrected Assignment SQL
    	String sql = "SELECT AVG(aa.percentage) FROM assignment_attempt aa " +
...
//            log.error("Error calculating assignment average: {}", e.getMessage());
            return 0.0;
        }
    }
}