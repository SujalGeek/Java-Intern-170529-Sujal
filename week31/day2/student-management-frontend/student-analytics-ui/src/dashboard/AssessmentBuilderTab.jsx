...
const AssessmentBuilderTab = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // We explicitly craft the EXACT payload each backend service expects.

      if (config.type === 'midterm') {
        // 🔥 Midterm Service (AiIntegrationService) expects snake_case: 'total_marks'
        ...
        // 🔥 Quiz Service expects only courseId and description
        const quizPayload = {
            courseId: parseInt(config.courseId),
            description: config.description
        };
        ...
        // 🔥 Assignment Service DTO expects camelCase: 'totalMarks' and 'difficulty'
        const assignmentPayload = {
            courseId: parseInt(config.courseId),
            description: config.description,
            totalMarks: parseInt(config.totalMarks), 
            difficulty: "medium" // Default difficulty for the DTO
        };
        
        // This is now correctly sending the payload in the BODY, not the URL!
