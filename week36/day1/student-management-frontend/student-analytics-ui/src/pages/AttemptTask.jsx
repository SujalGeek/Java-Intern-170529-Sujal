...
//         // 🔥 THE FIX: DATA NORMALIZER FOR STUDENT ATTEMPT
//         let rawQuestions = [];
//         if (Array.isArray(data)) rawQuestions = data;
//         else if (data.questions && Array.isArray(data.questions)) rawQuestions = data.questions;
//         else if (data.assignment && data.assignment.questions) rawQuestions = data.assignment.questions;
//         else if (data.quiz && data.quiz.questions) rawQuestions = data.quiz.questions;
        
//         // Standardize the questions so React always knows exactly what to print
//         const normalizedQuestions = rawQuestions.map(q => {
//             let parsedOpts = [];
//             if (q.optionsJson && q.optionsJson !== "null" && q.optionsJson !== "[]") {
//                 try {
//                     let raw = q.optionsJson;
//                     if (typeof raw === 'string') raw = JSON.parse(raw);
//                     if (typeof raw === 'string') raw = JSON.parse(raw); 
//                     if (Array.isArray(raw)) parsedOpts = raw;
//                 } catch(e){}
...
//             return {
//                 id: q.examQuestionId || q.questionId || q.id,
//                 text: q.questionText || q.question || "Question text missing",
//                 marks: q.marks || q.weightage || 10,
//                 options: parsedOpts
//             };
//         });

//         setTaskData({ ...data, questions: normalizedQuestions });
...
//            <div className="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
//               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Total Score</p>
//               <p className="text-5xl font-black text-indigo-600 tracking-tighter italic">{result.totalScore || result.score || "N/A"}</p>
//            </div>
//            <div className="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm">
//               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">AI Grade</p>
//               <p className="text-5xl font-black text-indigo-600 tracking-tighter italic">{result.grade || 'A'}</p>
...
//             Attempting <span className="text-indigo-600">{type}</span>
//           </h1>
//           <p className="text-slate-400 font-bold uppercase text-xs tracking-widest mt-4 opacity-70">Initializing Real-time Evaluation Engine</p>
//         </header>

//         <div className="space-y-10">
//           {/* 🔥 FALLBACK IF ASSIGNMENT HAS ZERO QUESTIONS IN DB */}
//           {(!taskData?.questions || taskData.questions.length === 0) ? (
//             <div className="bg-red-50 dark:bg-red-900/20 p-10 rounded-[3rem] border border-red-200 text-center">
//               <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
//               <h3 className="text-xl font-black text-red-600 uppercase tracking-widest">No Questions Found</h3>
//               <p className="text-sm font-bold text-red-500/70 mt-2">This assessment is empty in the database. Please inform your instructor.</p>
//             </div>
//           ) : (
//             taskData.questions.map((q, index) => (
//               <motion.div 
...
//                       {index + 1}
//                     </span>
//                     <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Question ID: {q.id}</p>
//                   </div>
//                   <span className="text-[10px] font-black text-indigo-600 uppercase bg-indigo-50 dark:bg-indigo-900/30 px-4 py-2 rounded-xl border border-indigo-100 dark:border-indigo-800/50 italic">
//                     Allocation: {q.marks} Credits
//                   </span>
//                 </div>
...
const AttemptTask = () => {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        ...
        // 🔥 DATA NORMALIZER FOR STUDENT ATTEMPT
        let rawQuestions = [];
        if (Array.isArray(data)) rawQuestions = data;
        else if (data.questions && Array.isArray(data.questions)) rawQuestions = data.questions;
        else if (data.assignment && data.assignment.questions) rawQuestions = data.assignment.questions;
  ...
  // 🔥 THE RESET LOGIC WITH SURGICAL 404 HANDLING
  const handleResetDb = async () => {
    if (!window.confirm("Are you sure you want to wipe your previous attempt? This cannot be undone.")) return;
    ...
    try {
      ...
      // 🔥 YOUR REQUESTED FIX: Handling the specific 404 case
      if (err.response && err.response.status === 404) {
          alert("Alert: There is no quiz attempt found for this student to reset.");
      } else {
          alert("Failed to clear DB. Ensure the Delete endpoint is added to Spring Boot.");
      }
    } finally {
      ...
      <div className="max-w-5xl mx-auto space-y-12 text-left">
        ...
        <div className="pt-10 pb-20 space-y-6">
          ...
          <button 
            ...
            className="w-full py-5 bg-transparent border-2 border-red-500/20 text-red-500 rounded-[2rem] font-bold uppercase tracking-widest hover:bg-red-500/10 transition-all flex items-center justify-center gap-3"
          >
             <RefreshCcw className="w-5 h-5" /> Dev Mode: Force Clear Attempt
          </button>
