...
#         # 🔥 COMMENTED OUT: We are bypassing Chroma's automated embedding
...
#         # 🔥 SAFETY NET: Wipe old collection so it never conflicts
#         try:
#             self.client.delete_collection(name=collection_name)
#             print(f"🧹 Wiped old Chroma collection for {collection_name} to start fresh.")
#         except Exception:
#             pass

#         # 🔥 MANUAL EMBEDDING: We encode it ourselves
...
#         # 🔥 NO embedding_function parameter. We are sending raw numbers.
...
#         collection.add(
#             documents=chunks,
#             embeddings=embeddings,    # 🔥 Pass the raw numbers explicitly
#             ids=ids
#         )
...
#         # 🔥 NO embedding_function parameter here either
#         collection = self.client.get_or_create_collection(
#             name=f"course_{course_id}"
#         )

#         # 🔥 MANUAL ENCODING: We encode the search query ourselves
...
class RAGEngine:
    ...
    def __init__(self):
        ...
        if os.path.exists(model_path):
            print("Found Baked-in Model. Loading locally...")
    ...
    #  FIX: HttpClient ke andar host aur Settings dono same hone chahiye
        self.client = chromadb.HttpClient(
            host=chroma_host,
    ...
    #  PROPER REWRITE OF GENERATE FUNCTION
    # Added require_json=False, mapped to Groq API Params
    def generate(self, prompt, require_json=False):
        print("Generating content using Groq LLM...")
        try:
            # Build the API payload dynamically
            api_params = {
                ...
                "messages": [
                    {
                        ...
                        "content": (
                            ...
                            # "3. If context is insufficient, say 'INSUFFICIENT CONTEXT'.\n"
                            # "4. Follow formatting exactly as instructed.\n"
                            # "5. You strictly output JSON when requested.\n"
                           "You are an elite academic exam generator. "
                            "Follow formatting exactly as instructed. You strictly output JSON when requested."
                        )
                    },
                    {
                        "role": "user",
            ...
            #  THE SILVER BULLET: Force Groq to return guaranteed JSON
            if require_json:
                api_params["response_format"] = {"type": "json_object"}

            response = self.groq_client.chat.completions.create(**api_params)
            ...
            print("Groq Generation Error:", str(e))
            # Return an empty JSON object if JSON was required, else an empty string
