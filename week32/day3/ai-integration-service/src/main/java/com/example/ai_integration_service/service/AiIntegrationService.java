...
//	            }
	 ...
	 // =========================
	    ...
	    BigDecimal midtermPercent = BigDecimal.ZERO;
	    if (exam.getTotalMarks() > 0) {
	    	midtermPercent = totalScore.multiply(BigDecimal.valueOf(100))
	    ...
	    try {
	        ...
	        Double qAvg = quizClient.getAverageForStudent(studentId, exam.getCourseId());
	        Double aAvg = assignmentClient.getAverageForStudent(studentId, exam.getCourseId());
	        ...
	        dto.setAttendancePercentage(calculateAttendance(studentId, exam.getCourseId()));
	        dto.setParticipationScore(calculateParticipation(studentId));
	        dto.setStudyHoursPerWeek(getStudyHours(studentId));
...
//	        / Your fallback
	    ...
	    try {
	        ...
	        log.error("Performance service sync failed", ex);
	    }
