...
//	}
 ...
 // 🔥 Feeds the QuestionBankTab with Draft/Published Status
	...
	public Double getAverageScoreForStudent(Long studentId, Long courseId) {
	    // We query the quiz_question (score) table linked through quiz_attempt
	    String sql = "SELECT AVG(qa.score) FROM quiz_question qa " +
	                 "JOIN quiz_attempt qat ON qa.attempt_id = qat.attempt_id " +
	                 "JOIN quiz q ON qat.quiz_id = q.id " +
	                 "WHERE qat.student_id = ? AND q.course_id = ?";
	    try {
	        Double avg = jdbcTemplate.queryForObject(sql, Double.class, studentId, courseId);
	        return (avg != null) ? avg : 0.0;
	    } catch (Exception e) {
//	        log.error("Error calculating quiz average: {}", e.getMessage());
	        return 0.0;
	    }
	}
