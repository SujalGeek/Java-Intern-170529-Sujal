...
//         const res = await api.get(`/api/exam-results/questions/course/${courseId}`);
...
//       setLoading(true);
//       try {
//         // 🔥 FIX: Pointing to the unified evaluation endpoint in QuizController
//         // Using v1 path to match your Backend RequestMapping
//         const res = await api.get(`/api/v1/exams/evaluations/course/${courseId}`);
        
//         // Ensure we handle both single objects and arrays safely
//         const data = Array.isArray(res.data) ? res.data : [res.data];
//         console.log("Grading Hub Data Received:", data);
//         setSubmissions(data);
//       } catch (err) {
//         console.error("Evaluation Sync Failed:", err);
//         setSubmissions([]); 
...
//   // --- 2. SEARCH FILTERING (Student ID or Question Text) ---
//   const filteredSubmissions = (submissions || []).filter(sub => {
//     const studentMatch = sub.studentId?.toString().includes(searchTerm);
//     // Mapping: sub.question is the key we use in Java Map
//     const questionMatch = sub.question?.toLowerCase().includes(searchTerm.toLowerCase());
...
//     if (!selectedScript || !newScore) return;
    
//     try {
//       // 1. Update the score in Database
//       await api.put(`/api/v1/exams/evaluations/${selectedScript.examResultId}/override`, { 
//           newScore: parseFloat(newScore) 
//       });

//       alert(`Success! Score overridden to ${newScore}. Recalculating AI Predictions...`);

//       // 2. DOMINO EFFECT: Trigger Performance Service to update ML & GPA
//       await api.post(`/api/performance/predict/${selectedScript.studentId}/${courseId}`);

//       // 3. Reset UI & Refresh List
//       setIsEditing(false);
//       // Pura page refresh kar le ya function call kar le to get fresh data
//       window.location.reload(); 

//     } catch (err) {
//       console.error("Failed to override:", err);
//       alert("Error saving override. Check console.");
//     }
//   };
...
//                     setNewScore(sub.score); // Set default score in input
//                   }}
//                   className={`w-full text-left p-5 rounded-2xl border-2 transition-all group ${
//                     selectedScript?.examResultId === sub.examResultId
//                       ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20'
...
//                     <p className="text-[9px] font-black uppercase tracking-widest opacity-60 mb-1">AI Score</p>
//                     <p className="text-4xl font-black italic tracking-tighter">
//                       {selectedScript.score}<span className="text-lg opacity-40">/{selectedScript.marks}</span>
//                     </p>
//                   </div>
//                 </div>
...
const GradingHubTab = ({ courseId }) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      // Trigger the ML recalculation for Finalizing
      await api.post(`/api/performance/predict/${selectedScript.studentId}/${courseId}`);
      alert(` Grade Finalized! Score of ${selectedScript.score} for Student ${selectedScript.studentId} is saved.`);
      
      // Remove it from the list if you want, or just deselect
      setSelectedScript(null);
    } catch(err) {
      console.error(err);
      alert("Error finalizing grade.");
    }
  };
  ...
  const handleSaveOverride = async () => {
    ...
    try {
      // 1. Update the score in Database
      await api.put(`/api/exam-results/evaluations/${selectedScript.examResultId}/override`, { 
          newScore: parseFloat(newScore) 
      });
            ...
            {loading ? (
              ...
              filteredSubmissions.map((sub, idx) => (
                <motion.button
                  ...
                  transition={{ delay: idx * 0.05 }}
                  onClick={() => { 
                    setSelectedScript(sub);
                    setIsEditing(false); // Reset edit state when new item clicked
                    setNewScore(sub.score); // Default new score to the AI's score
                  }}
                  className={`w-full text-left p-5 rounded-2xl border-2 transition-all group ${
                    selectedScript?.examResultId === sub.examResultId
                      ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20'
            ...
            {selectedScript ? (
              ...
              >
                ...
                <div className="flex justify-between items-start pb-8 border-b border-slate-100 dark:border-slate-800">
                  ...
                  {/* AI SCORE / INPUT BOX TOGGLE */}
                  <div className="text-center bg-slate-900 dark:bg-emerald-600 p-6 rounded-[2rem] text-white shadow-xl shadow-slate-200 dark:shadow-none min-w-[140px]">
                    <p className="text-[9px] font-black uppercase tracking-widest opacity-60 mb-1">AI Score</p>
                    {isEditing ? (
                      <div className="flex items-center justify-center gap-1 mt-2">
                        <input 
                          ...
                          max={selectedScript.marks}
                          min="0"
                          step="0.1"
                          autoFocus
                          className="w-20 text-center text-2xl font-black text-slate-900 bg-white rounded-lg py-1 outline-none"
                        />
                        <span className="text-lg opacity-60">/{selectedScript.marks}</span>
                      </div>
                    ) : (
                      <p className="text-4xl font-black italic tracking-tighter">
                        {selectedScript.score}<span className="text-lg opacity-40">/{selectedScript.marks}</span>
                ...
                {/* ACTION HUD (DYNAMIC BUTTONS) */}
                <div className="pt-10 flex gap-6">
                  {isEditing ? (
                    <>
                      <button 
                        onClick={(e) => { e.preventDefault(); handleSaveOverride(); }} 
                        className="flex-1 bg-amber-500 text-white font-black text-xs uppercase tracking-[0.3em] py-6 rounded-[1.5rem] hover:bg-amber-600 transition-all shadow-xl shadow-amber-200 dark:shadow-none flex items-center justify-center gap-2"
                      >
                         Save New Grade
                      </button>
                      <button 
                        onClick={(e) => { e.preventDefault(); setIsEditing(false); }} 
                        className="px-12 bg-slate-200 text-slate-600 font-black text-[10px] uppercase tracking-widest py-6 rounded-[1.5rem] hover:bg-slate-300 transition-all"
                      >
                        Cancel
                      </button>
                    </>
                    ...
                    <>
                      ...
                      <button 
                        ...
                        className="px-12 bg-white dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700 font-black text-[10px] uppercase tracking-widest py-6 rounded-[1.5rem] hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
                      >
                        Manual Override
                      </button>
                    </>
