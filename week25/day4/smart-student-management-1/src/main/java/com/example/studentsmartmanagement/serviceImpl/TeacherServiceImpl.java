...
public class TeacherServiceImpl implements TeacherService{
	...
	public void addCourse(CourseRequest request) {
		...
		{
			throw new BusinessException("Course Code: "+request.getCourseCode());
		}
	...
	public void assignGrade(GradeRequest request) {
		...
		Enrollment enrollment = enrollmentRepository.findById(request.getEnrollmentId())
				.orElseThrow(
						() -> new ResourceNotFoundException("\"Enrollment not found with ID:" + request.getEnrollmentId()));
	
	if(request.getScoreObtained() > request.getMaxScore())
	{
		throw new BusinessException("Score obtained cannot be greater than Max Score");
	}
