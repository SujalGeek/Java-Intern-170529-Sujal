...
//	    Map examData = (Map) body.get("exam");
	    ...
	    if (sectionA == null || sectionA.isEmpty())
	        throw new RuntimeException("MCQ generation failed");

	    if (sectionB == null || sectionB.isEmpty())
	        throw new RuntimeException("Short question generation failed");

	    if (sectionC == null || sectionC.isEmpty())
	        throw new RuntimeException("Long question generation failed");
	    
	    log.info("Section A size: {}", sectionA.size());
...
//	    saveDynamicSection(exam.getMidtermId(), sections.get("Section B - Short"), 5, courseId);
//	    saveDynamicSection(exam.getMidtermId(), sections.get("Section C - Long"), 10, courseId);
	    
	    saveDynamicSection(exam.getMidtermId(), sectionA, 2, courseId);
	    saveDynamicSection(exam.getMidtermId(), sectionB, 5, courseId);
	    saveDynamicSection(exam.getMidtermId(), sectionC, 10, courseId);
...
//	}
	...
	public byte[] exportMidtermPdf(Long midtermId) throws Exception {
	    ...
	    document.add(new Paragraph("Total Questions: " + exam.getTotalQuestions()));
	    document.add(new Paragraph(" "));
	    document.add(new Paragraph("--------------------------------------------------"));
	    ...
	    // SECTION A
	    document.add(new Paragraph("Section A - MCQ (2 Marks Each)")
	            .setBold());
	    document.add(new Paragraph(" "));

	    for (MidtermExamQuestion mapping : mappings) {

	        if (mapping.getMarks() == 2) {
	    ...
	    // SECTION B
	    document.add(new Paragraph("Section B - Short Answer (5 Marks Each)")
	            .setBold());
	    document.add(new Paragraph(" "));

	    for (MidtermExamQuestion mapping : mappings) {

	        if (mapping.getMarks() == 5) {
	    ...
	    // SECTION C
	    document.add(new Paragraph("Section C - Long Answer (10 Marks Each)")
	            .setBold());
	    document.add(new Paragraph(" "));

	    for (MidtermExamQuestion mapping : mappings) {

	        if (mapping.getMarks() == 10) {
...
//	        );
//	    }
	private void saveDynamicSection(Long midtermId,
	...
	String bloomLevel = questionMap.get("bloom_level").toString();
	

	if (questionMap.containsKey("reference_answer")) {
	referenceAnswer = questionMap.get("reference_answer").toString();
	}

	if (questionMap.containsKey("correct_answer")) {
	    referenceAnswer = questionMap.get("correct_answer").toString();
	}
